...
class TestConvertCli(TestCaseCompare):
    ...
    def test_excel_to_ndjson(self):
        os.system(f"python -m aito.cli.main_parser convert excel {self.input_folder}/sample.xlsx > {self.out_file_path}")
        self.assertCountEqual(ndjson.load(self.out_file_path.open()),
                              ndjson.load((self.input_folder / 'sample.ndjson').open()))

    def test_excel_one_sheet_to_ndjson(self):
        os.system(f"python -m aito.cli.main_parser convert excel {self.input_folder}/sample_multi_sheets.xlsx -o Sheet2 "
                  f"> {self.out_file_path}")
        self.assertCountEqual(ndjson.load(self.out_file_path.open()),
                              ndjson.load((self.input_folder / 'sample_id_reversed.ndjson').open()))

    def test_csv_to_json(self):
        os.system(f"python -m aito.cli.main_parser convert csv --json < {self.input_folder}/sample.csv > {self.out_file_path}")
    ...
    def test_excel_to_json(self):
        os.system(f"python -m aito.cli.main_parser convert excel {self.input_folder}/sample.xlsx --json > {self.out_file_path}")
        self.assertCountEqual(json.load(self.out_file_path.open()),
                              json.load((self.input_folder / 'sample.json').open()))

    def test_excel_one_sheet_to_json(self):
        os.system(f"python -m aito.cli.main_parser convert excel {self.input_folder}/sample_multi_sheets.xlsx -o Sheet2 -j"
                  f"> {self.out_file_path}")
        self.assertCountEqual(json.load(self.out_file_path.open()),
